...
export async function runDebugHarness(input: unknown, dependencies: HarnessDependencies = {}) {
  ...
  session.trace = [];
  const complete = {
    description: "Finish with a concise Ukrainian reply. For a sourced recipe, never say it is ready if a non-optional requirement was not added after alternate searches; explicitly name the unavailable requirements.",
