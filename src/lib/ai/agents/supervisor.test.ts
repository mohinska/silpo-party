...
describe("main supervisor", () => {
  ...
  it("does not block an actionable request just because the budget is missing", async () => {
    const result = await runSupervisorDecision({
      partyState: { hasIntent: true, hasBudget: false, hasProposal: false },
      message: "додай колу",
      generate: async () => JSON.stringify({
        actions: [{ type: "ask_user", question: "Вкажіть бюджет." }],
        reply: "Вкажіть бюджет.",
      }),
    });

    expect(result.actions.map((action) => action.type)).toEqual(["build_basket", "review_constraints"]);
    expect(result.reply).toContain("кошик");
  });
});
