...
export function createAgentV2Repository(transport?: RpcTransport) {
  ...
  function checkpointArgs(input: { jobId: string; workerId: string; fence: number; expectedInputRevision: number; expectedDraftRevision: number; expectedSourceRevision: number; eventProcessed?: boolean; stepSequence: number; workspace: Workspace; messages: unknown[]; checkpoint: Record<string, unknown>; publish?: boolean; activityCode?: ActivityCode; question?: { id: string; recipientId: string; content: string } | null }) {
  ...
  return {
    ...
    async checkpointAndAcknowledge(input: { jobId: string; workerId: string; fence: number; expectedInputRevision: number; expectedDraftRevision: number; expectedSourceRevision: number; eventProcessed?: boolean; stepSequence: number; workspace: Workspace; messages: unknown[]; checkpoint: Record<string, unknown>; publish?: boolean; activityCode?: ActivityCode; question?: { id: string; recipientId: string; content: string } | null; status: Exclude<RunStatus, "running">; nextAttemptAt?: string | null }) {
