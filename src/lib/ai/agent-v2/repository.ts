...
const ClaimSchema = z.object({ job_id: uuid, party_id: uuid, event_id: uuid, attempts: counter, fence: counter, input_revision: counter, draft_revision: counter, source_revision: counter, processed_source_revision: counter, step_sequence: counter, workspace: WorkspaceSchema, checkpoint: z.record(z.string(), z.unknown()) });
...
export function createAgentV2Repository(transport?: RpcTransport) {
  ...
  function checkpointArgs(input: { jobId: string; workerId: string; fence: number; expectedInputRevision: number; expectedDraftRevision: number; expectedSourceRevision: number; eventProcessed?: boolean; stepSequence: number; workspace: Workspace; messages: unknown[]; checkpoint: Record<string, unknown>; publish?: boolean; activityCode?: ActivityCode; question?: { id: string; recipientId: string; content: string } | null; notices?: { id: string; recipientId: string; content: string }[] | null }) {
    const workspace = WorkspaceSchema.parse(input.workspace);
    const question = input.question === undefined || input.question === null ? null : z.object({ id: uuid, recipientId: uuid, content: z.string().trim().min(1).max(1000) }).strict().parse(input.question);
    const notices = z.array(z.object({ id: uuid, recipientId: uuid, content: z.string().trim().min(1).max(1000) }).strict()).max(50).parse(input.notices ?? []);
    return { p_job_id: uuid.parse(input.jobId), p_worker_id: z.string().min(1).parse(input.workerId), p_fence: counter.parse(input.fence), p_expected_input: counter.parse(input.expectedInputRevision), p_expected_draft: counter.parse(input.expectedDraftRevision), p_expected_source: counter.parse(input.expectedSourceRevision), p_event_processed: input.eventProcessed ?? false, p_step: counter.parse(input.stepSequence), p_workspace: workspace, p_messages: z.array(z.unknown()).parse(input.messages), p_checkpoint: input.checkpoint, p_projection: input.publish ? projectDraft(workspace) : null, p_activity_code: input.activityCode ? ActivityCodeSchema.parse(input.activityCode) : null, p_question: question ? { id: question.id, recipientId: question.recipientId, content: question.content } : null, p_notices: notices.length ? notices : null };
  }
  return {
    ...
    async checkpoint(input: { jobId: string; workerId: string; fence: number; expectedInputRevision: number; expectedDraftRevision: number; expectedSourceRevision: number; eventProcessed?: boolean; stepSequence: number; workspace: Workspace; messages: unknown[]; checkpoint: Record<string, unknown>; publish?: boolean; activityCode?: ActivityCode; question?: { id: string; recipientId: string; content: string } | null; notices?: { id: string; recipientId: string; content: string }[] | null }) {
      await call("agent_v2_checkpoint", checkpointArgs(input));
    },
    async checkpointAndAcknowledge(input: { jobId: string; workerId: string; fence: number; expectedInputRevision: number; expectedDraftRevision: number; expectedSourceRevision: number; eventProcessed?: boolean; stepSequence: number; workspace: Workspace; messages: unknown[]; checkpoint: Record<string, unknown>; publish?: boolean; activityCode?: ActivityCode; question?: { id: string; recipientId: string; content: string } | null; notices?: { id: string; recipientId: string; content: string }[] | null; status: Exclude<RunStatus, "running">; nextAttemptAt?: string | null }) {
      const status = RunStatusSchema.exclude(["running"]).parse(input.status);
      const nextAttemptAt = input.nextAttemptAt == null ? null : z.iso.datetime({ offset: true }).parse(input.nextAttemptAt);
      await call("agent_v2_checkpoint_and_ack", { ...checkpointArgs(input), p_status: status, p_next_attempt_at: nextAttemptAt });
    },
    async acknowledge(input: { jobId: string; workerId: string; fence: number; stepSequence: number; status: Exclude<RunStatus, "running">; nextAttemptAt?: string | null }) {
      const status = RunStatusSchema.exclude(["running"]).parse(input.status);
      const nextAttemptAt = input.nextAttemptAt == null ? null : z.iso.datetime({ offset: true }).parse(input.nextAttemptAt);
      await call("agent_v2_ack", { p_job_id: uuid.parse(input.jobId), p_worker_id: z.string().min(1).parse(input.workerId), p_fence: counter.parse(input.fence), p_step: counter.parse(input.stepSequence), p_status: status, p_next_attempt_at: nextAttemptAt });
    },
