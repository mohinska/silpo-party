    ...
    <main className="page-shell align-start">
      <div className="workspace">
        ...
        <section className="panel stack">
          ...
          {isOpen && hostConnected && <form action={addItem.bind(null, party.code)} className="add-item-form"><label>Пошук товару в «Сільпо»<input name="name" required placeholder="Сир моцарела" /></label><label>Кількість<input name="quantity" type="number" min="0.01" max="10000" step="0.01" defaultValue="1" required /></label><label>Одиниця<input name="unit" defaultValue="шт." required /></label><label>Орієнтовна ціна, грн<input name="unit_price" type="number" min="0" step="0.01" defaultValue="0" required /></label><button className="primary-button">Знайти й додати</button></form>}
