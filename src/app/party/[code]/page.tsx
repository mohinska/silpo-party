    ...
    <section className="agent-proposal">
      ...
      {!proposal ? (
        ...
        <div className="proposal-sections">
          ...
          <section><h4>Рецепти та джерела</h4>{proposal.recipes.map((recipe) => <article key={recipe.id}>{recipe.source.url ? <a href={recipe.source.url} target="_blank" rel="noreferrer">{recipe.title} ↗</a> : <strong>{recipe.title}</strong>}<small>{recipe.source.provider === "silpo" ? "Рецепт Сільпо" : recipe.source.url ? "Наданий рецепт" : "Інгредієнти з деталей запиту"} · масштабовано до {recipe.baseServings} порц.</small></article>)}</section>
